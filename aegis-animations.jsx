// aegis-animations.jsx v3 — stick figure mortality scenes with watercolor washes

(function () {

  // ─── SHARED HELPERS ───────────────────────────────────────────────────────

  function useCycleTime(duration, offset) {
  ...
  function ph(t, s, e) { return c01((t - s) / (e - s)); }

  function wavePath(ys, x0, dx) {
    let d = `M${x0},${ys[0]}`;
  ...
  // Renders blurry irregular fills that simulate a watercolor wash.
  // Each scene calls this at the bottom of its SVG (drawn first = behind everything).

  function WcDefs({ id, seed = 5, scale = 12 }) {
    return (
      <defs>
        ...
        <filter id={`wc_edge_${id}`} x="-25%" y="-25%" width="150%" height="150%">
          <feTurbulence type="fractalNoise" baseFrequency="0.032 0.024"
            numOctaves="4" seed={seed} result="n"/>
  ...
  function WcBlob({ id, cx, cy, rx, ry, fill, opacity = 0.28, edge = false }) {
  ...
  // Sky + ground watercolor layer for outdoor scenes
  function OutdoorWash({ id, w = 280, h = 140, groundY = 108,
    skyColor = '#9EC8E8', groundColor = '#7ABF7A', skyOp = 0.32, groundOp = 0.38 }) {
    return (
      <>
        <WcDefs id={id} seed={parseInt(id, 36) % 20 + 2} scale={14}/>
        {/* Sky — two layered blobs for depth */}
        <WcBlob id={id} cx={140} cy={groundY * 0.38} rx={170} ry={groundY * 0.46}
          fill={skyColor} opacity={skyOp}/>
        <WcBlob id={id} cx={100} cy={groundY * 0.28} rx={90} ry={groundY * 0.22}
          fill={skyColor} opacity={skyOp * 0.5} edge/>
        <WcBlob id={id} cx={200} cy={groundY * 0.3} rx={70} ry={groundY * 0.18}
          fill={skyColor} opacity={skyOp * 0.4} edge/>
        {/* Ground strip */}
        <WcBlob id={id} cx={140} cy={groundY + 12} rx={160} ry={20}
          fill={groundColor} opacity={groundOp}/>
        <WcBlob id={id} cx={80}  cy={groundY + 8}  rx={90}  ry={14}
          fill={groundColor} opacity={groundOp * 0.55} edge/>
        <WcBlob id={id} cx={210} cy={groundY + 10} rx={80}  ry={13}
          fill={groundColor} opacity={groundOp * 0.5} edge/>
      </>
    );
  }

  // Interior wall + floor wash
  function InteriorWash({ id, w = 280, h = 140, floorY = 108,
    wallColor = '#C8D8E8', floorColor = '#D4C8A0' }) {
    return (
      <>
        <WcDefs id={id} seed={7} scale={10}/>
        <WcBlob id={id} cx={140} cy={floorY * 0.45} rx={165} ry={floorY * 0.5}
          fill={wallColor} opacity={0.3}/>
        <WcBlob id={id} cx={60}  cy={floorY * 0.3}  rx={80}  ry={floorY * 0.25}
          fill={wallColor} opacity={0.14} edge/>
        <WcBlob id={id} cx={140} cy={floorY + 14} rx={155} ry={22}
          fill={floorColor} opacity={0.34}/>
      </>
    );
  }

  // Water fill wash (for ocean/river scenes)
  function WaterWash({ id, w = 280, h = 140, waterY = 72,
    waterColor = '#5A9EC8', skyColor = '#A8D4EC' }) {
    return (
      <>
        <WcDefs id={id} seed={11} scale={12}/>
        {/* Sky above water */}
        <WcBlob id={id} cx={140} cy={waterY * 0.4} rx={170} ry={waterY * 0.5}
          fill={skyColor} opacity={0.26}/>
        {/* Water body */}
        <WcBlob id={id} cx={140} cy={waterY + 34} rx={165} ry={40}
          fill={waterColor} opacity={0.22}/>
        <WcBlob id={id} cx={90}  cy={waterY + 20} rx={90}  ry={20}
          fill={waterColor} opacity={0.16} edge/>
        <WcBlob id={id} cx={200} cy={waterY + 22} rx={75}  ry={18}
          fill={waterColor} opacity={0.14} edge/>
      </>
    );
  }
  ...
  function PianoScene({ ink, accent }) {
    ...
    return (
      <svg viewBox="0 0 280 140" width="100%" height="140"
        ...
        {/* Watercolor background — interior hall */}
        <InteriorWash id="piano" floorY={108} wallColor="#C2D4E8" floorColor="#D4C8A8"/>

        <defs>
          <filter id="sk1">
            <feTurbulence type="fractalNoise" baseFrequency="0.07 0.05"
        ...
        {/* Ground */}
        <path d="M6,108 Q70,107 140,108 Q210,109 274,108"
          stroke={ink} strokeWidth="2.5" fill="none" strokeLinecap="round"/>
        {[68,142,214].map(x =>
          <path key={x} d={`M${x},108 L${x},115`}
            stroke={ink} strokeWidth="1.2" fill="none" opacity="0.22"/>
        )}
        ...
        <g clipPath="url(#gc1)" filter="url(#sk1)">
          <g transform={`translate(${figX},108) scale(${scaleX},${scaleY})`}>
            ...
            <path d={`M0,-46 Q1,-32 0,-18`}
              stroke={ink} strokeWidth="2.5" fill="none" strokeLinecap="round"/>
            <path d={`M0,-38 Q${-7+swing*0.3},-32 ${-13+swing*0.45},-24`}
              stroke={ink} strokeWidth="2.2" fill="none" strokeLinecap="round"/>
            <path d={`M0,-38 Q${7-swing*0.3},-32 ${13-swing*0.45},-24`}
              stroke={ink} strokeWidth="2.2" fill="none" strokeLinecap="round"/>
            <path d={`M0,-18 Q${-5+swing*0.4},-9 ${-9+swing*0.55},0`}
              stroke={ink} strokeWidth="2.5" fill="none" strokeLinecap="round"/>
            <path d={`M0,-18 Q${5-swing*0.4},-9 ${9-swing*0.55},0`}
              stroke={ink} strokeWidth="2.5" fill="none" strokeLinecap="round"/>
          </g>
        ...
        {t > 0.22 && !inScatter && (
          <g transform={`translate(${figX},${pianoY})`} filter="url(#sk1)">
            {pianoP < 0.75 && [-28, -6, 18].map((x, i) =>
              <line key={i} x1={x} y1="-52" x2={x - 2} y2={-64 - i * 4}
                stroke={ink} strokeWidth="1" opacity="0.28"/>
            )}
            <path d="M-36,-46 Q-40,-22 -36,0 L34,0 Q40,-12 38,-30 Q32,-46 -36,-46 Z"
              stroke={ink} strokeWidth="2.5" fill="none" strokeLinejoin="round"/>
            <path d="M-36,-46 Q-8,-70 38,-30"
              stroke={ink} strokeWidth="2.5" fill="none"/>
            <line x1="-2" y1="-46" x2="6" y2="-60" stroke={ink} strokeWidth="1.8"/>
            <path d="M-24,0 L-24,-14 Q-4,-16 20,-14 L20,0"
              stroke={ink} strokeWidth="2" fill="none"/>
            {[-18, -11, -4, 3, 10, 16].map((kx, i) =>
              <line key={i} x1={kx} y1="-14" x2={kx} y2="0"
                stroke={ink} strokeWidth="1.1" opacity="0.5"/>
            )}
            {[-15, -1, 13].map((kx, i) =>
              <rect key={i} x={kx} y="-14" width="5" height="8"
        ...
        {inScatter && PIECES.map((p, i) => {
          ...
          return (
            <g key={i} transform={`translate(${px},${py}) rotate(${ang})`}
               opacity={Math.max(0, 1 - scatP * 1.05)} filter="url(#sk1)">
              <rect x={-p.w / 2} y={-p.h / 2} width={p.w} height={p.h}
                stroke={ink} strokeWidth="1.5" fill="none" rx="1"/>
            </g>
          );
        })}

        {inScatter && (
          <g transform={`translate(${figX},108)`}
             opacity={Math.max(0, 1 - scatP * 1.3)}>
            {[-28, -12, 0, 14, 28].map((x, i) =>
              <ellipse key={i} cx={x} cy={-scatP * 10} rx={7 + i * 3} ry="3"
                stroke={ink} strokeWidth="1" fill="none" opacity="0.3"/>
            )}
          </g>
        )}
      </svg>
  ...
  function OceanScene({ ink, accent }) {
    ...
    return (
      <svg viewBox="0 0 280 140" width="100%" height="140"
        ...
        {[8, 16, 24].map((dy, i) => {
          ...
          return (
            <path key={i} d={wavePath(uy, 0, 14)}
              stroke={ink} strokeWidth="1" fill="none" opacity={0.08 - i * 0.02}/>
          );
        })}
        ...
        <g clipPath="url(#gc2above)" filter="url(#sk2)" opacity={figBodyOp}>
          <g transform={`translate(${figX},${figY})`}>
            ...
            <path d={`M16,0 Q28,${-10 + Math.sin(strokeCycle) * 5} ${40 + Math.cos(strokeCycle) * 8},${-4 + Math.sin(strokeCycle) * 10}`}
              stroke={ink} strokeWidth="2.2" fill="none" strokeLinecap="round"/>
            <path d={`M4,0 Q${-8 + Math.cos(strokeCycle + Math.PI) * 10},${4 + Math.sin(strokeCycle + Math.PI) * 5} ${-18},${6}`}
              stroke={ink} strokeWidth="2.2" fill="none" strokeLinecap="round"/>
            <path d={`M-20,0 Q-24,${3 + Math.sin(strokeCycle * 2) * 7} -30,${7 + Math.sin(strokeCycle * 2) * 8}`}
              stroke={ink} strokeWidth="2" fill="none" strokeLinecap="round"/>
            <path d={`M-20,0 Q-24,${-2 + Math.sin(strokeCycle * 2 + 1.2) * 7} -30,${-5 + Math.sin(strokeCycle * 2 + 1.2) * 8}`}
              stroke={ink} strokeWidth="2" fill="none" strokeLinecap="round"/>
            {[0, 1, 2].map(i => (
              <circle key={i} cx={28 + i * 5} cy={-14 - i * 3} r="1.8"
                fill={ink} opacity={0.28 + Math.sin(strokeCycle + i) * 0.15}/>
            ))}
          </g>
        </g>
        ...
        {t > 0.44 && (
          <g transform={`translate(${bigWaveX},0)`} filter="url(#sk2)">
            <path d={`M0,${waterY} Q22,${waterY - bigWaveH} 55,${waterY - bigWaveH * 0.5} Q85,${waterY - 10} 110,${waterY}`}
              stroke={ink} strokeWidth="2.5" fill="none"/>
            <path d={`M0,${waterY} Q22,${waterY - bigWaveH} 55,${waterY - bigWaveH * 0.5} Q85,${waterY - 10} 110,${waterY} L110,140 L0,140 Z`}
              fill={ink} opacity="0.07"/>
            {[4, 14, 26, 40].map((x, i) => (
              <path key={i} d={`M${x + 3},${waterY - bigWaveH * 0.7 + i * 5} Q${x + 8},${waterY - bigWaveH * 0.85 + i * 5} ${x + 16},${waterY - bigWaveH * 0.7 + i * 5}`}
                stroke={ink} strokeWidth="1.3" fill="none" opacity="0.32"/>
            ))}
          </g>
        )}

        {calmP > 0 && [0, 1, 2].map(i => (
          <ellipse key={i} cx={lerp(38, 168, 0.5)} cy={waterY + 4}
            rx={6 + i * 16 + calmP * 12} ry={3 + i * 1.5}
            stroke={ink} strokeWidth="1" fill="none"
            opacity={Math.max(0, 0.38 - i * 0.1 - calmP * 0.32)}/>
        ))}
      </svg>
    );
  }
  ...
  function BridgeScene({ ink, accent }) {
    ...
    return (
      <svg viewBox="0 0 280 140" width="100%" height="140"
           style={{ opacity, display: 'block', overflow: 'visible' }}>
        {/* Watercolor sky + gorge color */}
        <OutdoorWash id="bridge" groundY={deckY} skyColor="#A8C8E4" groundColor="#8AB880"
          skyOp={0.28} groundOp={0.3}/>
        {/* Gorge water tint */}
        <WcDefs id="bridgegorge" seed={13} scale={10}/>
        <WcBlob id="bridgegorge" cx={(gapL+gapR)/2} cy={gorgeB + 8} rx={30} ry={12}
          fill="#4A8EC4" opacity={0.3}/>

        <defs>
          <filter id="sk3">
            <feTurbulence type="fractalNoise" baseFrequency="0.065 0.05"
              numOctaves="2" seed="13" result="n"/>
        ...
        <path d={`M${gapL},${deckY} L${gapL - 4},${gorgeB + 4} L${gapL},${gorgeB + 4}`}
          stroke={ink} strokeWidth="1.5" fill={ink} opacity="0.06" strokeLinejoin="round"/>
        <path d={`M${gapR},${deckY} L${gapR + 4},${gorgeB + 4} L${gapR},${gorgeB + 4}`}
          stroke={ink} strokeWidth="1.5" fill={ink} opacity="0.06" strokeLinejoin="round"/>
        {[6, 14, 22].map(dy => (
          <line key={dy} x1={gapL - 3} y1={deckY + dy} x2={gapL - 1} y2={deckY + dy + 4}
            stroke={ink} strokeWidth="0.8" opacity="0.12"/>
        ))}

        <rect x={gapL} y={gorgeB} width={gapR - gapL} height={10} fill={ink} opacity="0.06"/>
        <path d={wavePath(gorgeWaveYs, gapL, (gapR - gapL) / (NUM_W - 1))}
          stroke={ink} strokeWidth="1.4" fill="none" opacity="0.3"/>
        ...
        {showCar && (
          <g transform={`translate(${carX},${carY}) rotate(${carAng})`}
             filter="url(#sk3)" clipPath="url(#gc3full)">
        ...
        {t < 0.50 && driveP > 0.08 && [0, 1, 2].map(i => (
          <line key={i} x1={carX - 32 - i * 14} y1={deckY - 5 + i * 7}
            x2={carX - 46 - i * 14} y2={deckY - 5 + i * 7}
            stroke={ink} strokeWidth="1" opacity={0.22 - i * 0.05}/>
        ))}
        ...
        {splashP > 0 && [0, 36, 72, 108, 145, 182, 218, 252, 290, 324].map((deg, i) => {
          ...
          return (
            <line key={i}
              x1={(gapL + gapR) / 2 + Math.cos(rad) * 4}
              y1={gorgeB + 3 + Math.sin(rad) * 2}
              x2={(gapL + gapR) / 2 + Math.cos(rad) * r}
              y2={gorgeB + 3 + Math.sin(rad) * r * 0.38}
              stroke={ink} strokeWidth="1.5"
              opacity={Math.max(0, 1 - splashP * 1.1)}/>
          );
        })}
        {splashP > 0.4 && [0, 1].map(i => (
          <ellipse key={i} cx={(gapL + gapR) / 2} cy={gorgeB + 3}
            rx={4 + (splashP - 0.4) * 30 + i * 10} ry={2 + (splashP - 0.4) * 5}
            stroke={ink} strokeWidth="1" fill="none"
            opacity={Math.max(0, 0.4 - (splashP - 0.4) * 1.2 - i * 0.1)}/>
        ))}
      </svg>
    );
  }
  ...
  function PlaneScene({ ink, accent }) {
    const t = useCycleTime(6800);
    const resetP = ph(t, 0.93, 1.0);
    const opacity = resetP > 0 ? lerp(1, 0, resetP) : 1;
    ...
    function Jet({ x, y, angle }) {
      return (
        <g transform={`translate(${x},${y}) rotate(${angle})`} filter="url(#sk4)">
          <path d="M-40,0 Q-40,-7 -16,-9 Q10,-10 36,-5 Q46,0 36,5 Q10,10 -16,9 Q-40,7 -40,0 Z"
          ...
          {[18, 8, -2, -12, -22].map((wx, i) => (
            <ellipse key={i} cx={wx} cy="-4" rx="3.5" ry="3"
              stroke={ink} strokeWidth="1.1" fill="none" opacity="0.55"/>
          ))}
          <path d="M10,-9 Q2,-22 -12,-32 L-24,-30 Q-18,-16 -20,-9"
            stroke={ink} strokeWidth="2.2" fill="none" strokeLinejoin="round"/>
          <path d="M10,9 Q2,22 -12,32 L-24,30 Q-18,16 -20,9"
            stroke={ink} strokeWidth="2.2" fill="none" strokeLinejoin="round"/>
          <path d="M-8,-28 Q-14,-34 -20,-28 Q-14,-22 -8,-28"
            stroke={ink} strokeWidth="1.8" fill="none"/>
          <path d="M-8,28 Q-14,34 -20,28 Q-14,22 -8,28"
            stroke={ink} strokeWidth="1.8" fill="none"/>
          <line x1="-8" y1="-28" x2="-5" y2="-28" stroke={ink} strokeWidth="2" strokeLinecap="round"/>
          <line x1="-8" y1="28" x2="-5" y2="28" stroke={ink} strokeWidth="2" strokeLinecap="round"/>
          <path d="M-40,0 L-46,-22 Q-44,-24 -38,-18 L-36,-9"
            stroke={ink} strokeWidth="2" fill="none" strokeLinejoin="round"/>
          <path d="M-36,-9 L-46,-18 L-40,-18 L-34,-9"
            stroke={ink} strokeWidth="1.6" fill="none" strokeLinejoin="round"/>
          <path d="M-36,9 L-46,18 L-40,18 L-34,9"
            stroke={ink} strokeWidth="1.6" fill="none" strokeLinejoin="round"/>
          <rect x="-30" y="-9" width="7" height="9" stroke={ink} strokeWidth="0.9"
            fill="none" rx="1" opacity="0.32"/>
        </g>
      );
    }
    ...
    return (
      <svg viewBox="0 0 280 140" width="100%" height="140"
           style={{ opacity, display: 'block', overflow: 'visible' }}>
        {/* Watercolor sky + ground */}
        <OutdoorWash id="plane" groundY={108} skyColor="#A0C8E8" groundColor="#88B878"
          skyOp={0.34} groundOp={0.32}/>

        <defs>
          <filter id="sk4">
            <feTurbulence type="fractalNoise" baseFrequency="0.065 0.05"
              numOctaves="2" seed="17" result="n"/>
            <feDisplacementMap in="SourceGraphic" in2="n" scale="2.2"
              xChannelSelector="R" yChannelSelector="G"/>
          </filter>
        </defs>

        {/* Clouds — slightly colored watercolor blobs */}
        ...
        <path d="M0,108 Q80,107 140,108 Q200,109 280,108"
          stroke={ink} strokeWidth="2" fill="none" opacity="0.28"/>
        {[12, 44, 80, 222, 252].map((x, i) => (
          <g key={i} opacity="0.14" filter="url(#sk4)">
            <line x1={x} y1="108" x2={x} y2={98} stroke={ink} strokeWidth="2.2"/>
            <path d={`M${x - 7},98 Q${x},87 ${x + 7},98 Z`}
              stroke={ink} strokeWidth="1.6" fill="none"/>
          </g>
        ))}

        {t > 0.06 && t < 0.38 && (
          <path d={`M${Math.max(8, baseX - 60)},${planeY} L${baseX - 8},${planeY}`}
            stroke={ink} strokeWidth="1.2" fill="none" opacity="0.14" strokeDasharray="5 8"/>
        )}
        {diveP > 0 && !showImpact && (
          <path d={`M${baseX},40 Q${swoopX - 18},${planeY - 12} ${swoopX},${planeY}`}
            stroke={ink} strokeWidth="1" fill="none" opacity="0.11" strokeDasharray="3 5"/>
        )}
        ...
        {showFire && [0, 1, 2, 3].map(i => {
          ...
          const fy = engY + Math.sin(tailDirRad) * (6 + i * 8)
            + Math.sin(i * 2.1 + t * 50) * 3.5;
          return (
            <ellipse key={i} cx={fx} cy={fy}
              rx={Math.max(0.5, 5.5 - i * 1.1)}
              ry={Math.max(0.5, 3.2 - i * 0.65)}
        ...
        {showImpact && [0,24,48,72,96,120,148,174,200,228,255,282,310,338].map((deg, i) => {
          ...
          return (
            <line key={i}
              x1={swoopX + Math.cos(rad) * 3} y1={108 + Math.sin(rad) * 2}
              x2={swoopX + Math.cos(rad) * r} y2={108 + Math.sin(rad) * r * 0.4}
              stroke={accent} strokeWidth="2" opacity={Math.max(0, 1 - impactP * 1.05)}/>
          );
        })}
      </svg>
    );
  ...
  function MortalityRow() {
    const th  = window.__aegisTheme || {};
    const ink    = th.ink    || '#0B1C2C';
    const accent = th.accent || '#1A6B9A';
    const surface = th.surface || '#C8D9ED';
    const border  = th.border  || '#8AAAC8';
    const muted   = th.muted   || '#4A6B8A';
  ...
  function SceneCard({ scene, style }) {
    const th = window.__aegisTheme || {};
    const ink    = th.ink    || '#0B1C2C';
    const accent = th.accent || '#1A6B9A';
    const border  = th.border  || '#8AAAC8';
    ...
    return (
      <div style={{ background: '#FAFBFC', border: `2px solid ${border}`,
        borderRadius: '3px 10px 3px 10px / 10px 3px 10px 3px',
        overflow: 'hidden', padding: '8px 6px 2px', ...style }}>
        <Comp ink={ink} accent={accent}/>
      </div>
    );
  }
  ...
  Object.assign(window, {
    MortalityRow, SceneCard,
    PianoScene, OceanScene, BridgeScene, PlaneScene,
    WcDefs, WcBlob, OutdoorWash, InteriorWash, WaterWash,
  });
