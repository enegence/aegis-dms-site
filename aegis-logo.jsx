...
function AegisMascot({ height = 80, color = 'currentColor' }) {
  return (
    <svg width={height * 2.4} height={height} viewBox="0 0 192 80" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Left upstroke, diagonal down to middle, diagonal back up, right downstroke */}
      <path
...
function AegisLockup({ size = 'md', color = 'currentColor' }) {
  const heights = { sm: 36, md: 56, lg: 80 };
  const h = heights[size] || 56;
  const tagSizes = { sm: 10, md: 13, lg: 16 };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 4 }}>
      ...
      <div style={{
        ...
        paddingLeft: 2,
      }}>Digital Legacy System</div>
    </div>
  );
}

Object.assign(window, { AegisMascot, AegisLockup });
