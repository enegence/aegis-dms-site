...
<!-- DEVIATION (2026-05-17): AegisLockup now additionally renders a hand-drawn "AEGIS" badge stamp above the mascot (uses `useTheme()` for accent underlines). It is the new canonical lockup, byte-identical across BOTH repos, and supersedes the "byte-identical to $BACKUP/aegis-logo.jsx" constraint. AegisMascot is unchanged/identical. See spec for full rationale. -->
